...
#include "parser.h" // substitute for gpl.tab.h
#include "error.h"
...
{
/*  cerr << "about to invoke game_object\n";
  Game_object::all_game_objects();
  cerr << "about to invoke table_handler\n";
  Table_handler::instance();
  cerr << "done\n";*/
  ...
  // if -DGRAPHICS was specified when compiling gpl.cpp then include this code
    ...
    if (symbol != nullptr)
    {
      ...
      else window_width = symbol->as_int();
    }
    ...
    if (symbol != nullptr)
    {
      ...
      else window_height = symbol->as_int();
    }
    ...
    if (symbol != nullptr)
    {
      ...
      else window_red = symbol->as_double();
    }
    ...
    if (symbol != nullptr)
    {
      ...
      else window_green = symbol->as_double();
    }
    ...
    if (symbol != nullptr)
    {
      ...
      else window_blue = symbol->as_double();
    }
    ...
    if (symbol != nullptr)
    {
      ...
      else window_title = symbol->as_string();
    }
    ...
    if (symbol != nullptr)
    {
      ...
      else animation_speed = symbol->as_int();
    }
  #endif
